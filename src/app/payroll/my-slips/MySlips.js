...
export default function MySlips({ userId }) {
  ...
  const [recreateLoading, setRecreateLoading] = useState(null);
  ...
  const handleDelete = async (slipId) => {
    ...
    setRecreateLoading(slipId); // Reuse recreateLoading for delete
    try {
            ...
            >
              ...
              <div className="flex gap-2">
                ...
                {session?.user?.role === "admin" && (
                  ...
                  >
                    <RefreshCw
                      className={`w-4 h-4 ${recreateLoading === slip._id ? "animate-spin" : ""}`}
                    />
                  </button>
                )}
                {session?.user?.role === "admin" && (
