...
export default function SlipView({ slipId }) {
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-8 print:p-0 print:bg-white">
            ...
            <div
                ref={slipRef}
                className="max-w-[210mm] min-h-[297mm] mx-auto bg-white shadow-lg print:shadow-none p-12 print:p-8 text-sm flex flex-col"
            >

                <div className="border-b-2 border-gray-200 pb-6 mb-8 mt-5">
                    <div className="flex flex-col items-center">
                        ...
                        <p className="text-gray-500 mt-1 whitespace-pre-line text-sm text-center">
                            <span className="font-semibold text-gray-600">Reg. Office:</span> {company?.address || "Company Address, City, State, Zip Code\nContact: contact@company.com"}
                        </p>
                ...
                <div className="grid grid-cols-2 gap-x-12 gap-y-4 mb-8">
                    <div className="space-y-3">
                        <div className="flex justify-between border-b border-dashed border-gray-200 pb-1">
                            ...
                            <span className="font-semibold text-gray-900">{slip.userId.name}</span>
                        </div>
                        <div className="flex justify-between border-b border-dashed border-gray-200 pb-1">
                            <span className="text-gray-500">Employee ID:</span>
                            <span className="font-semibold text-gray-900">
                                {slip.userId.employeeId || slip.userId._id.slice(-6).toUpperCase()}
                            </span>
                        </div>
                        <div className="flex justify-between border-b border-dashed border-gray-200 pb-1">
                            <span className="text-gray-500">Designation:</span>
                            <span className="font-semibold text-gray-900">{slip.userId.designation || "N/A"}</span>
                        </div>
                        <div className="flex justify-between border-b border-dashed border-gray-200 pb-1">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex justify-between border-b border-dashed border-gray-200 pb-1">
                            ...
                            <span className="font-semibold text-gray-900">{formatDate(slip.userId.joiningDate)}</span>
                        </div>
                        <div className="flex justify-between border-b border-dashed border-gray-200 pb-1">
                            <span className="text-gray-500">Effective Work Days:</span>
                            <span className="font-semibold text-gray-900">{slip.presentDays}</span>
                        </div>
                        <div className="flex justify-between border-b border-dashed border-gray-200 pb-1">
                            <span className="text-gray-500">Loss of Pay (Days):</span>
                            <span className={`font-semibold ${slip.lopDays > 0 ? "text-red-600" : "text-gray-900"}`}>{slip.lopDays}</span>
                        </div>
                    </div>
                </div>

                {/* Salary Table */}
                <div className="border border-gray-300 rounded overflow-hidden mb-8 flex-grow">
                    <div className="grid grid-cols-2 bg-gray-50 border-b border-gray-300 font-bold text-gray-700 uppercase text-xs tracking-wider">
                        <div className="p-3 text-center border-r border-gray-300">Earnings</div>
                        ...
                        <div className="p-0 flex flex-col h-full">
                            <table className="w-full">
                                <thead className="bg-gray-50 border-b border-gray-200">
                                    <tr>
                                        <th className="text-left p-2 pl-4 text-xs text-gray-500 font-medium">Component</th>
                                        <th className="text-right p-2 pr-4 text-xs text-gray-500 font-medium">Amount (₹)</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-100">
                                    ...
                                    {renderRow("Distance Allowance", slip.earnings.distanceAllowance)}
                            ...
                            {/* Filler to push footer down if table is short */}
                ...
                <div className="flex justify-end mb-8">
                    <div className="bg-blue-50 border border-blue-100 p-4 rounded-lg w-1/2">
                        <div className="flex justify-between items-center mb-1">
                            <span className="text-sm font-semibold text-blue-800 uppercase">Net Payable</span>
                            <span className="text-2xl font-bold text-gray-900">₹{formatCurrency(slip.netPay)}</span>
                        </div>
                ...
                {/* Amount in Words (Fallback for print clarity) */}
                <div className="mb-12 hidden print:block">
                    <p className="text-sm font-semibold text-gray-800 border-b border-gray-200 pb-1">
                        Amount in Words: <span className="italic font-normal">{toWords(slip.netPay)} Only</span>
                    </p>
                </div>
...
function renderRow(label, amount) {
    ...
    return (
        <tr className="hover:bg-gray-50 transition-colors">
            ...
            <td className="p-2 pr-4 text-right font-medium text-gray-900">₹{amount.toLocaleString("en-IN")}</td>
        </tr>
    );
}
