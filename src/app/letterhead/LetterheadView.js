...
import {
    Upload, Printer, ChevronLeft, ChevronRight, Menu, ZoomIn, ZoomOut, RotateCcw,
    ArrowUpDown, AlignVerticalJustifyCenter, ChevronDown, AlignLeft, AlignCenter,
    AlignRight, AlignJustify, Type, Heading1, Heading2, Heading3,
    Bold, Italic, Underline, Strikethrough, List, ListOrdered, Indent, Outdent,
    Link, Image, Subscript, Superscript, Quote, Code, Eraser, Table as TableIcon
} from "lucide-react";
...
if (typeof window !== "undefined") {
    try {
        ...
        if (Quill) {
            ...
            const LineHeightStyle = new StyleAttributor('line-height', 'line-height', { scope: Scope.BLOCK });
            LineHeightStyle.whitelist = LINE_HEIGHTS;
            Quill.register(LineHeightStyle, true);

            const SpacingStyle = new StyleAttributor('spacing', 'margin-bottom', { scope: Scope.BLOCK });
...
const ToolbarDropdown = ({ value, options, onChange, icon: Icon, placeholder, width = "w-24", contentWidth, className = "" }) => {
        ...
        <div className={`relative h-8 ${width} ${className}`} ref={wrapperRef}>
            ...
            >
                {Icon ? (
                    ...
                    <>
                        <span className="truncate flex-grow text-left">{selectedLabel}</span>
                        <ChevronDown className={`w-3 h-3 text-gray-400 flex-shrink-0 transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`} />
                    </>
                )}
            </div>

            {isOpen && (
                <div className={`absolute top-full left-0 mt-1 ${contentWidth || 'w-max min-w-full'} bg-white border border-gray-200 rounded-md shadow-lg z-[100] max-h-48 overflow-y-auto flex flex-col`}>
                    {options.map((opt) => (
                        <button
                            key={opt.value}
                            type="button"
...
const FontSizeTool = ({ quillRef, currentFormat }) => {
    ...
    const changeSize = (direction) => {
        ...
        if (currentIndex === -1) {
            ...
            if (direction === 1 && currentIndex < whitelist.length - 1) newSize = whitelist[currentIndex + 1];
            if (direction === -1 && currentIndex > 0) newSize = whitelist[currentIndex - 1];
        }
    ...
    return (
        <div className="flex items-center gap-0 border border-gray-200 rounded-md bg-white h-8 overflow-visible shadow-sm relative" ref={wrapperRef}>
            <button type="button" onClick={() => changeSize(-1)} className="hover:bg-gray-100 h-full px-2 text-gray-500 font-medium border-r border-gray-100 rounded-l"> - </button>
            ...
            <button type="button" onClick={() => changeSize(1)} className="hover:bg-gray-100 h-full px-2 text-gray-500 font-medium border-l border-gray-100 rounded-r"> + </button>
        </div>
    );
};
...
const CustomToolbar = ({ zoom, zoomIn, zoomOut, resetZoom, quillRef }) => {
    ...
    const insertTable = () => {
        if (quillRef?.current) {
            ...
            if (range) {
                const tableHTML = '<table style="border-collapse: collapse; width: 100%; border: 1px solid #ccc;"><tbody><tr><td style="border: 1px solid #ccc; padding: 8px;">Cell 1</td><td style="border: 1px solid #ccc; padding: 8px;">Cell 2</td></tr><tr><td style="border: 1px solid #ccc; padding: 8px;">Cell 3</td><td style="border: 1px solid #ccc; padding: 8px;">Cell 4</td></tr></tbody></table>';
    ...
    return (
        <div id="toolbar" className="flex flex-col gap-4 ">
            ...
            <div className="flex flex-col gap-1 w-full">
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider text-center">Edit & Zoom</span>
                <div className="flex flex-wrap justify-between gap-1 bg-gray-50 p-2 rounded border border-gray-200">
                    ...
                    <div className="flex items-center gap-1 border-l pl-2 border-gray-300">
                        <button type="button" onClick={zoomOut} className="p-1 hover:bg-gray-200 rounded"><ZoomOut className="w-3 h-3 text-gray-600" /></button>
                        <span className="text-[10px] font-mono w-8 text-center">{Math.round(zoom * 100)}%</span>
                        <button type="button" onClick={zoomIn} className="p-1 hover:bg-gray-200 rounded"><ZoomIn className="w-3 h-3 text-gray-600" /></button>
                        <button type="button" onClick={resetZoom} className="p-1 hover:bg-gray-200 rounded"><RotateCcw className="w-3 h-3 text-gray-400" /></button>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-col gap-1 w-full">
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider text-center">Typography</span>
            ...
            {/* Style Section */}
            <div className="flex flex-col gap-1 w-full">
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider text-center">Style</span>
                <div className="flex flex-wrap justify-center gap-1 bg-gray-50 p-2 rounded border border-gray-200">
                    <ToolbarButton onClick={() => toggleFormat('bold')} isActive={currentFormat.bold} title="Bold"><Bold className="w-4 h-4" /></ToolbarButton>
                    <ToolbarButton onClick={() => toggleFormat('italic')} isActive={currentFormat.italic} title="Italic"><Italic className="w-4 h-4" /></ToolbarButton>
                    <ToolbarButton onClick={() => toggleFormat('underline')} isActive={currentFormat.underline} title="Underline"><Underline className="w-4 h-4" /></ToolbarButton>
                    <ToolbarButton onClick={() => toggleFormat('strike')} isActive={currentFormat.strike} title="Strikethrough"><Strikethrough className="w-4 h-4" /></ToolbarButton>
            ...
            <div className="flex flex-col gap-1 w-full">
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider text-center">Paragraph</span>
                <div className="flex flex-wrap justify-center gap-1 bg-gray-50 p-2 rounded border border-gray-200 items-center">
                    ...
                    <div className="flex items-center gap-0.5 border-r pr-2 border-gray-300 mr-2">
                        <ToolbarButton onClick={() => handleFormat('align', '')} isActive={!currentFormat.align} title="Align Left"><AlignLeft className="w-4 h-4" /></ToolbarButton>
                        <ToolbarButton onClick={() => handleFormat('align', 'center')} isActive={currentFormat.align === 'center'} title="Align Center"><AlignCenter className="w-4 h-4" /></ToolbarButton>
                        <ToolbarButton onClick={() => handleFormat('align', 'right')} isActive={currentFormat.align === 'right'} title="Align Right"><AlignRight className="w-4 h-4" /></ToolbarButton>
                        <ToolbarButton onClick={() => handleFormat('align', 'justify')} isActive={currentFormat.align === 'justify'} title="Justify"><AlignJustify className="w-4 h-4" /></ToolbarButton>
                    </div>

                    <div className="flex gap-0.5 mr-2">
                        <button className="ql-list p-1 hover:bg-gray-200 rounded" value="ordered"></button>
                        <button className="ql-list p-1 hover:bg-gray-200 rounded" value="bullet"></button>
                    </div>

                    <div className="flex gap-0.5 mr-2 border-l pl-2 border-gray-300">
                        <button className="ql-indent p-1 hover:bg-gray-200 rounded" value="-1"></button>
                        <button className="ql-indent p-1 hover:bg-gray-200 rounded" value="+1"></button>
                    </div>
            ...
            {/* Insert Section */}
            <div className="flex flex-col gap-1 w-full">
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider text-center">Insert</span>
                <div className="flex flex-wrap justify-center gap-1 bg-gray-50 p-2 rounded border border-gray-200">
        ...
        <div className="min-h-screen bg-gray-100 p-8 print:p-0 print:bg-white text-gray-900 font-sans relative transition-all duration-300">
            ...
            >
                ...
                <div className="pb-10 flex flex-col gap-6">
                    ...
                    {/* Custom Toolbar */}
                    <div>
                        <CustomToolbar zoom={zoom} zoomIn={zoomIn} zoomOut={zoomOut} resetZoom={resetZoom} quillRef={quillRef} />
                    </div>
                </div>
            </div>
                ...
                <div className="max-w-[210mm] w-[210mm] min-h-[297mm] bg-white shadow-2xl print:shadow-none print:w-full print:max-w-none print:m-0 flex flex-col px-12 py-10 relative box-border">
                    ...
                    <header className="flex justify-between items-start border-b-2 border-blue-500 pb-2 mb-5">
                        ...
                        <div className="text-right text-xs text-gray-500 leading-relaxed">
                            {/* Contact Block */}
                            <div className="mb-3">
                                <span className="uppercase" style={{ color: profile.formatting?.color, fontWeight: "bold" }}>Contact Us</span>
                    ...
                    <div className="flex-grow relative group min-h-[850px]">
                        <ReactQuill
                            ref={quillRef}
                            theme="snow"
                            value={content}
                    ...
                    <footer className="mt-auto pt-2 border-t-2 border-blue-500 text-xs text-gray-500 text-center">
                        <div className="flex justify-between gap-40 mb-2 text-left">
                            {/* Registered Office */}
                            <div>
                                <h4 className="font-bold uppercase text-gray-800 mb-1" style={{ color: profile.formatting?.color }}>Registered Office</h4>
                                <p className="whitespace-pre-line leading-relaxed">{profile?.address}</p>
                            </div>
                            {/* Registration Details */}
                            <div>
                                <h4 className="font-bold uppercase text-gray-800 mb-1" style={{ color: profile.formatting?.color }}>Registration</h4>
