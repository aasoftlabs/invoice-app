...
export async function POST(req) {
    try {
        ...
        const { date, type, category, amount, description, paymentMode, reference } = body;
    ...
    try {
        ...
        if (transaction.reference?.type === "Invoice" && transaction.reference.id) {
            ...
            if (invoice) {
                ...
                invoice.paymentHistory = invoice.paymentHistory.filter(p => !p.transactionId.equals(transaction._id));
        ...
        } else if (transaction.reference?.type === "SalarySlip" && transaction.reference.id) {
    ...
    try {
        ...
        if (transaction.reference?.type === "Invoice" && transaction.reference.id) {
            ...
            if (invoice) {
                ...
                const historyIndex = invoice.paymentHistory.findIndex(p => p.transactionId.equals(transaction._id));
                if (historyIndex > -1) {
                    invoice.paymentHistory[historyIndex].amount = newAmount;
                    invoice.paymentHistory[historyIndex].date = transaction.date;
                    invoice.paymentHistory[historyIndex].note = `Payment via Accounts: ${paymentMode}`;
                }
