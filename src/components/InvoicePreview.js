  ...
  React.useEffect(() => {
    const handleResize = () => {
      if (containerRef.current) {
        ...
        // A4 width in pixels at standard DPI is ~794px (210mm)
        // We add some buffer for padding
        const standardWidth = 794;

        if (containerWidth < standardWidth) {
          const newScale = (containerWidth - 32) / standardWidth; // 32px padding buffer
          setScale(newScale);
        } else {
          setScale(1);
        }
                  ...
                  <div className="mt-4 pt-4 border-t-2 border-dashed border-gray-100 flex justify-between items-start">
                    ...
                    <div className="w-2/5 text-right">
                      {data.type === "Digital" ? (
                        ...
                        <>
                          <div
                            className="text-sm font-bold mb-12"
                            style={{
                              color: profile.formatting?.color || "#1d4ed8",
                          ...
                          <div
                            className="text-[10px] font-bold uppercase border-t pt-1 w-48 text-center text-gray-500"
                            style={{
